...
**Available Plots** (*"cookd", "ls", "lev" only available for "lm" and "glm" models*)
...
- `qqbands` = include 95% confidence bands on qq plot?
- `qqline` = include a 1-1 line on qq plot?
...
```{r fig.height = 6, fig.width = 8}
resid_panel(tree_model, plots = "R") # for lm and glm models only
```
...
frameWidget(interact_default, height = '110%')
```
...
**plot options, residual types, and format options**: same as `resid_panel`
...
- `plots`: same as `resid_panel`
- `type`: residual types (same as `resid_panel`)
- `smoother`: indicates whether to add a smoother
- `qqband`: adds a confidence band to the qqplot
- format options: same as `resid_panel` (expect no `nrow`)

```{r fig.height = 3.5}
resid_compare(list(penguin_model, penguin_model_log2), 
              plots = c("resid"), smoother = TRUE, theme = "gray")
```
...
- [GitHub](https://github.com/goodekat/ggResidpanel)
