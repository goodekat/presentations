...
survival_data <- read_csv("data/deer_age_and_survival.csv")
...
betas <- read_csv("data/deer_betas.csv", col_names = FALSE) %>% as.matrix()
betas
```
...
vcov <- read_csv("data/deer_vcov.csv", col_names = FALSE) %>% as.matrix()
vcov
```
...
- Site (2 levels): Rural or urban 

- Time (21 levels): Year as a factor from 1999-2019
...
& & + \beta_{r,Urban} \times I[\mbox{rite = urban}]\\
& & + \beta_{r,2000} \times I[\mbox{year = 2000}]\\
& & + \beta_{r,2001} \times I[\mbox{year = 2001}]\\
& & \vdots\\
...
& & + \beta_{S,2000} \times I[\mbox{year = 2000}]\\
& & + \beta_{S,2001} \times I[\mbox{year = 2001}]\\
& & \vdots\\
...
- Writing functions in R to apply the delta method is a good when many cases
