...
# Transition: <font color="#8B16CE"> States </font>
...
> It is often better to set it explicetly when animating, to make sure your data is interpreted in the right way. 
...
# Transition: <font color="#AE6845"> Time </font>
...
Try adjusting the number of frames using `animate` (more on this later)
...
# Transition: <font color="#80BA54"> Reveal </font>
...
Use the ggimage R package to replace the poing with an image
...
Using the transitions learned so far, create the animation shown below.
...
Follows the data but switches between being static and changing to the range of the data (works will with `transistion_states`)

.left-code[
```{r view_step, eval = FALSE, echo = TRUE}
bats %>%
  ggplot(aes(x = longitude, 
             y = latitude, 
             color = time, 
...
.right-plot[
```{r view_step_out, ref.label = "view_step", echo = FALSE, cache = TRUE, fig.height = 6, fig.width = 7}
```
]
...
.left-code[
```{r view_zoom, eval = FALSE, echo = TRUE}
bats %>%
  ggplot(aes(x = longitude, 
             y = latitude, 
             color = time, 
  ...
  transition_states(states = id) +
  view_zoom(pause_length = 5, #<<
            step_length = 3, #<<
            nsteps = 5) #<<
```
]
.right-plot[
```{r view_zoom_out, ref.label = "view_zoom", echo = FALSE, cache = TRUE, fig.height = 6, fig.width = 7}
```
]
